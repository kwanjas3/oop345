...
public:
 ...
 // map is basically a vector where indexes are arbituary and assignable
 where the key is unique until multimap.
 
 //you can put a
 
// for(...............)
//  []{define func}(); // immediately call function
// auto f = [] {define function}, f();
 
 
int main(){
 for(int i = 0; i < 2; i++){
 static auto f = [&i] () { cout << i << '\n';}, f(i); // this doesnt work 
 }
}
